...
class UDPPose8Streamer {
  public:
  
  UDPPose8Streamer(const std::string & udp_address,
  ...
  : dgram_client(LIBSOCKET_IPv4), 
    ...
    sub = nh.subscribe("pose_topic", 1, &UDPPose8Streamer::PublishPose, this);
  }
  
  virtual ~UDPPose8Streamer() { dgram_client.destroy(); }
  ...
  void PublishPose(const mav_msgs::Actuators& msg) {
    ...
    if(accept){
        ...
        static const size_t pose_size = sizeof(float) * 8;
...
void wCallback(const mav_msgs::Actuators& cmd)
{
  ...
  Arr[5]=cmd.angular_velocities[5];
  Arr[6]=cmd.angular_velocities[6];
  Arr[7]=cmd.angular_velocities[7];

  std::cout <<"Message received: "<< Arr[0] << " "  << Arr[1] << " "<< Arr [2] << " "  << Arr[3] << " "<< Arr[4] << " "  << Arr[5] << " "<< Arr [6] << " "  << Arr[7] << " " << "\n";
...
{
  ros::init(argc, argv, "w8_to_unity");
  ...
  UDPPose8Streamer streamer(ip_address, port, offset_x, offset_y);
